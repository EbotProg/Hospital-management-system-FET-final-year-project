export const CREATE_REPORT_SUCCESS = " CREATE_REPORT_SUCCESS";
export const CREATE_REPORT_ERROR = "CREATE_REPORT_ERROR";
export const CREATE_REPORT_REQUEST = "CREATE_REPORT_REQUEST";

export const CREATE_CONSULTATION_INFO_SUCCESS = "CREATE_CONSULTATION_INFO_SUCCESS";
export const CREATE_CONSULTATION_INFO_ERROR = "CREATE_CONSULTATION_INFO_ERROR";
export const CREATE_CONSULTATION_INFO_REQUEST = "CREATE_CONSULTATION_INFO_REQUEST";

export const CREATE_LAB_REPORT_SUCCESS = " CREATE_LAB_REPORT_SUCCESS";
export const CREATE_LAB_REPORT_ERROR = "CREATE_LAB_REPORT_ERROR";
export const CREATE_LAB_REPORT_REQUEST = "CREATE_LAB_REPORT_REQUEST";

export const GET_DOCTOR_SUCCESS = " GET_DOCTOR_SUCCESS";
export const GET_DOCTOR_ERROR = "GET_DOCTOR_ERROR";
export const GET_DOCTOR_REQUEST = "GET_DOCTOR_REQUEST";

export const GET_APPOINTMENT_SUCCESS = " GET_APPOINTMENT_SUCCESS";
export const GET_APPOINTMENT_ERROR = "GET_APPOINTMENT_ERROR";
export const GET_APPOINTMENT_REQUEST = "GET_APPOINTMENT_REQUEST";

export const ADD_PATIENT_SUCCESS = " ADD_PATIENT_SUCCESS";
export const ADD_PATIENT_ERROR = "ADD_PATIENT_ERROR";
export const ADD_PATIENT_REQUEST = "ADD_PATIENT_REQUEST";

export const ADMIT_PATIENT_SUCCESS = " ADMIT_PATIENT_SUCCESS";
export const ADMIT_PATIENT_ERROR = "ADMIT_PATIENT_ERROR";
export const ADMIT_PATIENT_REQUEST = "ADMIT_PATIENT_REQUEST";


export const ADD_BED_SUCCESS = " ADD_BED_SUCCESS";
export const ADD_BED_ERROR = "ADD_BED_ERROR";
export const ADD_BED_REQUEST = "ADD_BED_REQUEST";

export const ADD_WARD_SUCCESS = "ADD_WARD_SUCCESS";
export const ADD_WARD_ERROR = "ADD_WARD_ERROR";
export const ADD_WARD_REQUEST = "ADD_WARD_REQUEST";

export const GET_ALL_WARDS_REQUEST = "GET_ALL_WARDS_REQUEST";
export const GET_ALL_WARDS_SUCCESS = "GET_ALL_WARDS_SUCCESS";
export const GET_ALL_WARDS_ERROR = "GET_ALL_WARDS_ERROR";



export const ADD_ROOM_SUCCESS = "ADD_ROOM_SUCCESS";
export const ADD_ROOM_ERROR = "ADD_ROOM_ERROR";
export const ADD_ROOM_REQUEST = "ADD_ROOM_REQUEST";

export const GET_BED_SUCCESS = " GET_BED_SUCCESS";
export const GET_BED_ERROR = "GET_BED_ERROR";
export const GET_BED_REQUEST = "GET_BED_REQUEST";

export const CREATE_BOOKING_SUCCESS = " CREATE_BOOKING_SUCCESS";
export const CREATE_BOOKING_ERROR = "CREATE_BOOKING_ERROR";
export const CREATE_BOOKING_REQUEST = "CREATE_BOOKING_REQUEST";

export const CREATE_PAYMENT_SUCCESS = " CREATE_PAYMENT_SUCCESS";
export const CREATE_PAYMENT_ERROR = "CREATE_PAYMENT_ERROR";
export const CREATE_PAYMENT_REQUEST = "CREATE_PAYMENT_REQUEST";

export const ADD_BEDS_SUCCESS = " ADD_BEDS_SUCCESS";
export const ADD_BEDS_ERROR = "ADD_BEDS_ERROR";
export const ADD_BEDS_REQUEST = "ADD_BEDS_REQUEST";

export const GET_SINGLE_BEDS_SUCCESS = " GET_SINGLE_BEDS_SUCCESS";
export const GET_SINGLE_BEDS_ERROR = "GET_SINGLE_BEDS_ERROR";
export const GET_SINGLE_BEDS_REQUEST = "GET_SINGLE_BEDS_REQUEST";

export const EDIT_SINGLE_BEDS_SUCCESS = " EDIT_SINGLE_BEDS_SUCCESS";
export const EDIT_SINGLE_BEDS_ERROR = "EDIT_SINGLE_BEDS_ERROR";
export const EDIT_SINGLE_BEDS_REQUEST = "EDIT_SINGLE_BEDS_REQUEST";

export const DISCHARGE_PATIENT_SUCCESS = " DISCHARGE_PATIENT_SUCCESS";
export const DISCHARGE_PATIENT_ERROR = "DISCHARGE_PATIENT_ERROR";
export const DISCHARGE_PATIENT_REQUEST = "DISCHARGE_PATIENT_REQUEST";

export const GET_PATIENTS_SUCCESS = " GET_PATIENTS_SUCCESS";
export const GET_PATIENTS_ERROR = "GET_PATIENTS_ERROR";
export const GET_PATIENTS_REQUEST = "GET_PATIENTS_REQUEST";

export const GET_PATIENT_SUCCESS = " GET_PATIENT_SUCCESS";
export const GET_PATIENT_ERROR = "GET_PATIENT_ERROR";
export const GET_PATIENT_REQUEST = "GET_PATIENT_REQUEST";

export const GET_ADMISSION_REPORT_SUCCESS = " GET_ADMISSION_REPORT_SUCCESS";
export const GET_ADMISSION_REPORT_ERROR = "GET_ADMISSION_REPORT_ERROR";
export const GET_ADMISSION_REPORT_REQUEST = "GET_ADMISSION_REPORT_REQUEST";

export const GET_APPOINTMENT_DETAILS_SUCCESS =
  " GET_APPOINTMENT_DETAILS_SUCCESS";
export const GET_APPOINTMENT_DETAILS_ERROR = "GET_APPOINTMENT_DETAILS_ERROR";
export const GET_APPOINTMENT_DETAILS_REQUEST =
  "GET_APPOINTMENT_DETAILS_REQUEST";

export const GET_ALLDATA_SUCCESS = " GET_ALLDATA_SUCCESS";
export const GET_ALLDATA_REQUEST = "GET_ALLDATA_REQUEST";

export const GET_ALL_NURSES_SUCCESS = " GET_ALL_NURSES_SUCCESS";
export const GET_ALL_NURSES_REQUEST = "GET_ALL_NURSES_REQUEST";

export const GET_ALL_DOCTORS_SUCCESS = " GET_ALL_DOCTORS_SUCCESS";
export const GET_ALL_DOCTORS_REQUEST = "GET_ALL_DOCTORS_REQUEST";

export const GET_ALL_DOCTORS_IN_WARD_SUCCESS = " GET_ALL_DOCTORS_IN_WARD_SUCCESS";
export const GET_ALL_DOCTORS_IN_WARD_REQUEST = "GET_ALL_DOCTORS_IN_WARD_REQUEST";

export const GET_ALL_NURSES_IN_WARD_SUCCESS = " GET_ALL_NURSES_IN_WARD_SUCCESS";
export const GET_ALL_NURSES_IN_WARD_REQUEST = "GET_ALL_NURSES_IN_WARD_REQUEST";

export const GET_ALL_ROOMS_IN_WARD_SUCCESS = " GET_ALL_ROOMS_IN_WARD_SUCCESS";
export const GET_ALL_ROOMS_IN_WARD_REQUEST = "GET_ALL_ROOMS_IN_WARD_REQUEST";

export const GET_ALL_BEDS_IN_WARD_SUCCESS = " GET_ALL_BEDS_IN_WARD_SUCCESS";
export const GET_ALL_BEDS_IN_WARD_REQUEST = "GET_ALL_BEDS_IN_WARD_REQUEST";

export const GET_ALL_ADMINS_SUCCESS = " GET_ALL_ADMINS_SUCCESS";
export const GET_ALL_ADMINS_REQUEST = "GET_ALL_ADMINS_REQUEST";

export const GET_ALL_ROOMS_SUCCESS = " GET_ALL_ROOMS_SUCCESS";
export const GET_ALL_ROOMS_REQUEST = "GET_ALL_ROOMS_REQUEST";

export const GET_ALL_BEDS_SUCCESS = " GET_ALL_BEDS_SUCCESS";
export const GET_ALL_BEDS_REQUEST = "GET_ALL_BEDS_REQUEST";

export const UPDATE_DOCTOR_AVAILABILITY_SUCCESS = " UPDATE_DOCTOR_AVAILABILITY_SUCCESS";
export const UPDATE_DOCTOR_AVAILABILITY_REQUEST = "UPDATE_DOCTOR_AVAILABILITY_REQUEST";

export const UPDATE_NURSE_AVAILABILITY_SUCCESS = " UPDATE_NURSE_AVAILABILITY_SUCCESS";
export const UPDATE_NURSE_AVAILABILITY_REQUEST = "UPDATE_NURSE_AVAILABILITY_REQUEST";

export const GET_ALL_ADMISSION_REPORTS_SUCCESS = " GET_ALL_ADMISSION_REPORTS_SUCCESS";
export const GET_ALL_ADMISSION_REPORTS_REQUEST = "GET_ALL_ADMISSION_REPORTS_REQUEST";

export const GET_ALL_DISCHARGE_REPORTS_SUCCESS = " GET_ALL_DISCHARGE_REPORTS_SUCCESS";
export const GET_ALL_DISCHARGE_REPORTS_REQUEST = "GET_ALL_DISCHARGE_REPORTS_REQUEST";

export const GET_ALL_DOCTOR_APPOINTMENTS_SUCCESS = " GET_ALL_DOCTOR_APPOINTMENTS_SUCCESS";
export const GET_ALL_DOCTOR_APPOINTMENTS_REQUEST = "GET_ALL_DOCTOR_APPOINTMENTS_REQUEST";

export const DELETE_APPOINTMENT_SUCCESS = " DELETE_APPOINTMENT_SUCCESS";
export const DELETE_APPOINTMENT_REQUEST = "DELETE_APPOINTMENT_REQUEST";

export const CANCEL_APPOINTMENT_SUCCESS = " CANCEL_APPOINTMENT_SUCCESS";
export const CANCEL_APPOINTMENT_REQUEST = "CANCEL_APPOINTMENT_REQUEST";

export const DELETE_WARD_SUCCESS = " DELETE_WARD_SUCCESS";
export const DELETE_WARD_REQUEST = "DELETE_WARD_REQUEST";

export const DELETE_ROOM_SUCCESS = " DELETE_ROOM_SUCCESS";
export const DELETE_ROOM_REQUEST = "DELETE_ROOM_REQUEST";

export const DELETE_BED_SUCCESS = " DELETE_BED_SUCCESS";
export const DELETE_BED_REQUEST = "DELETE_BED_REQUEST";

export const GET_REPORTS_SUCCESS = " GET_REPORTS_SUCCESS";
export const GET_REPORTS_REQUEST = "GET_REPORTS_REQUEST";

export const GET_LAB_REPORTS_SUCCESS = " GET_LAB_REPORTS_SUCCESS";
export const GET_LAB_REPORTS_REQUEST = "GET_LAB_REPORTS_REQUEST";

export const GET_MEDICAL_HISTORY_SUCCESS = " GET_MEDICAL_HISTORY_SUCCESS";
export const GET_MEDICAL_HISTORY_REQUEST = "GET_MEDICAL_HISTORY_REQUEST";
export const GET_MEDICAL_HISTORY_ERROR = "GET_MEDICAL_HISTORY_ERROR";

export const SEARCH_MEDICAL_HISTORY_SUCCESS = " SEARCH_MEDICAL_HISTORY_SUCCESS";
export const SEARCH_MEDICAL_HISTORY_REQUEST = "SEARCH_MEDICAL_HISTORY_REQUEST";
export const SEARCH_MEDICAL_HISTORY_ERROR = "SEARCH_MEDICAL_HISTORY_ERROR";

export const DOWNLOAD_MEDICAL_HISTORY_SUCCESS = " DOWNLOAD_MEDICAL_HISTORY_SUCCESS";
export const DOWNLOAD_MEDICAL_HISTORY_REQUEST = "DOWNLOAD_MEDICAL_HISTORY_REQUEST";

export const SEARCH_PATIENTS_SUCCESS = " SEARCH_PATIENTS_SUCCESS";
export const SEARCH_PATIENTS_REQUEST = "SEARCH_PATIENTS_REQUEST";
